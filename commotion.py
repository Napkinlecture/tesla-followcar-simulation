...
VIEW_DISTANCE = 10  # Distance for boid rules
...
class Agent:
    def __init__(self, x, y, speed, color):
        ...
        self.speed = speed
        self.color = color
        self.direction = np.array([0, 0])  # Initial direction
...
def get_directions(agent1, agent2, agent3):
    P = agent1.position - agent2.position  # Hunter 1 to Prey
    Q = agent1.position - agent3.position  # Hunter 2 to Prey
    R = agent2.position - agent3.position  # Hunter 2 to Hunter 1

    # Normalize the direction vector P (from Hunter 1 to Hunter 2)
    if np.linalg.norm(P) < np.linalg.norm(Q) and np.linalg.norm(P) != 0:
        dir_h1 = P / np.linalg.norm(P)  # Normalization to unit vector
    else:
        dir_h1 = Q / np.linalg.norm(Q) if np.linalg.norm(Q) != 0 else np.array([0, 0])  # Normalization

    # Hunter 2 direction should also be a normalized vector
    ...
    dir_prey = Q / np.linalg.norm(Q) - R / np.linalg.norm(R) if np.linalg.norm(R) != 0 else np.array([0, 0])  # Normalization
...
def main():
    ...
    hunter1 = Agent(x=initial_positions[0][0], y=initial_positions[0][1], speed=b_1, color=(255, 255, 0))  # Yellow
    hunter2 = Agent(x=initial_positions[1][0], y=initial_positions[1][1], speed=b_2, color=(0, 255, 0))  # Green
    prey = Agent(x=initial_positions[2][0], y=initial_positions[2][1], speed=b_3, color=(0, 255, 0))  # Green
    ...
    while running:
        ...
        # Apply boid rules to hunters and prey
        agents = [hunter1, hunter2, prey]
        for agent in agents:
            separation_force, alignment_force, cohesion_force = calculate_boid_rules(agent, agents)
            agent.direction += separation_force + alignment_force + cohesion_force
